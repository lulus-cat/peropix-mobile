...
뽑기가 끝나면 작가 다섯 명을 그림과 함께 권합니다. ★**그때가 다음 판을 궁리하는
순간**이라 그 자리에서 담아 두면 다음 판에 바로 씁니다 (뽑기 전에 띄우면 하려던 일을
가로막습니다).

- 근거는 **지금 프롬프트의 내용 태그**입니다 — 「이 태그를 실제로 그리는 작가」를
  Danbooru 에 물어봅니다(`related_tag`). ★눈금은 **그 작가 작업의 몇 할이 이 태그를
  달고 있는가**입니다. 전체에서 차지하는 비중으로 줄 세우면 매번 똑같은 대형 작가만
  올라옵니다
- 이미 서랍에 있는 사람, 장수가 너무 적은 사람(태그는 맞아도 NAI 가 모릅니다),
  작가가 아닌 태그는 뺍니다
- 「다른 작가」로 다시 뽑고, 「담기」로 바로 서랍에 넣습니다
- ★**깎기 중에는 안 뜹니다.** 한창 범인을 좁히는 중에 다른 작가를 들이밀면 그 판이
  흐트러집니다. 중지했거나 한 장도 못 만들었을 때도 뜨지 않습니다
- 「다시 보지 않기」로 끕니다
...
| `test_danbooru.js` | Danbooru 조회 (주소·응답 해석·반영율·통계·장르·추천) | 88 |
