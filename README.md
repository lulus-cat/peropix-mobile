...
- **세기 범위와 간격을 직접 정합니다.** 기본은 0.6 ~ 1.4, 0.05 간격입니다 (1.5 를 넘으면
  그 작가가 그림을 잡아먹고 0.5 아래면 없는 것과 다르지 않아서). 더 넓게 쓰시려면
  「세기 범위 정하기」에서 바꾸면 슬라이더와 훑을 눈금이 함께 따라갑니다.
  ★범위를 좁히면 지금 섞고 있는 값도 바로 끌어들입니다 — 안 그러면 화면에 보이는 값과
  실제로 나가는 값이 어긋납니다
...
| `test_artists.js` | 작가 서랍 (갈래·장르·세기 범위 설정·배분·굽기) | 64 |
