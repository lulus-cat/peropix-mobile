...
// ★빌드가 실수로 낸 태그에 끌려가면 안 된다. 두 저장소가 같은 워크플로를 나눠 쓰다가
//   실행 횟수 차이로 v1.2.-15 가 나온 적이 있다. '-' 를 구분자로 쪼개면 이것이
//   [1,2,15] 가 되어 1.2.12 보다 새 판이 되고, 앱이 영영 "업데이트 있음" 을 띄운다.
check('★음수가 든 태그는 낮은 판으로 읽는다 (새 판으로 착각하면 안 된다)',
  U.compare('1.2.-15', '1.2.12') === -1, String(U.compare('1.2.-15', '1.2.12')));
check('★그런 태그로는 업데이트를 안 알린다',
  U.decide({ current: '1.2.12', latest: { version: '1.2.-15' } }).show === false);
check('꼬리표 붙은 판은 숫자만 본다 (1.2.3-beta → 1.2.3)',
  U.compare('1.2.3-beta', '1.2.3') === 0, String(U.compare('1.2.3-beta', '1.2.3')));
check('꼬리표가 있어도 큰 판은 큰 판', U.compare('1.3.0-rc1', '1.2.9') === 1);
